...
class SysUserServiceImpl : SysUserService {
    ...
    override fun createSysUser(realUsername: String, realPassword: String, creator: String): ApiResult<*> {
        ...
        val sysRoles: MutableSet<String> = HashSet()
        sysRoles.add(PlayerType.USERS_CLIENT.name)

        val theRandom = ((Math.random() * 9 + 1) * 1000).toLong()
        val wid = SecurityConstants.USER_ID_PREFIX + Date().time + theRandom.toString()
        val password = MyUserKeyService.aesEncrypt(realPassword)
        val sysUser = SysUser(creator = creator, updater = creator, wid = wid, username = realUsername, password = password, sysroles = sysRoles)
        sysUserRepository.save(sysUser)
        return ResultUtil.success()
    }

    override fun setSysUser(username: String, password: String, wid: String, sysroles: MutableList<String>, updater: String): ApiResult<*> {
        ...
        sysuser.sysroles = sysroles.toHashSet()
    ...
    override fun adminGetUsers(playerType: String, pageSize: Int, pageIndex: Int): ApiResult<*> {
        ...
        if (PlayerType.ADMIN_SUPER.name in currentUser.roles) {
            val pattern = Pattern.compile("$playerType.+", Pattern.CASE_INSENSITIVE)
            val query = Query(Criteria.where("sysroles").`in`(pattern))
            val numSkip = pageIndex.toLong() * pageSize
            val totalCount = mongoTemplate.count(query, SysUser::class.java)
            val sysusers = mongoTemplate.find(query.with(Sort(Sort.Direction.DESC, "update_time")).skip(numSkip).limit(pageSize), SysUser::class.java)
            return ResultUtil.success(num = totalCount, data = sysusers)
        }
    ...
    override fun adminSetUser(username: String, password: String, wid: String?, sysroles: MutableList<String>): ApiResult<*> {
        ...
        if (PlayerType.ADMIN_SUPER.name in currentUser.roles) {
            ...
            return setSysUser(username = realUsername, password = password, wid = wid, sysroles = sysroles, updater = creator)
        }
        return ResultUtil.failure(msg = "非管理员权限")
    }
